...
def test_every_published_interpretation_strips_without_residue():
    ...
    for path in files:
        ...
        for field in ("reading", "counter_reading", "next_check"):
            ...
            # 접속사 `및`은 명사 사이에 홀로 서는 것이 정상이라 여기 넣지 않는다.
            assert not re.search(r"\s(부터|까지|의|은|는|이|가|를|을)\s", got), (
                f"{where}: 인용을 지운 자리에 조사가 남았다 — {got[:120]}")
            # 절 첫머리에 조사가 오는 것도 같은 증상이다.
            assert not re.search(r"(^|[.!?]\s+)(부터|까지|와|과|및|의|은|는|이|가|를|을)\s", got), (
                f"{where}: 절이 조사로 시작한다 — {got[:120]}")
            assert not re.search(r"[-~–—]\s*[가-힣]", got), (
                f"{where}: 찌꺼기 기호가 남았다 — {got[:120]}")
            assert "  " not in got and "()" not in got, f"{where}: 빈 괄호/이중 공백"
