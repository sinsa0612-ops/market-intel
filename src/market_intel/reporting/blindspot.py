...
SECTOR_WATCH: dict[str, list[str]] = {
    ...
    # 2026-08-20 커버리지 보강분. 셋은 관측 기업이 **있어서** "관측 없음"이 아니라
    # "업종은 튀었는데 우리 기업은 조용"이라는 더 강한 판정이 가능하다.
    "139260.KS": ["005930.KS", "000660.KS"],   # IT <- 삼성전자·SK하이닉스
    "091180.KS": ["005380.KS"],                # 자동차 <- 현대차
    "091170.KS": ["105560.KS"],                # 은행 <- KB금융
    "117700.KS": [],                           # 건설 — 관측 0곳
    "228810.KS": [],                           # 미디어 — 관측 0곳
    "266370.KS": [],                           # 2차전지 — 관측 0곳
    # --- 미국 ---
    ...
    # --- 규모 축 (2026-08-20) ---
    #
    # 업종이 아니라 **크기**로 가른 축이다. SPDR 11개는 전부 S&P500(대형주)
    # 기반이라, 소형주가 통째로 움직여도 11개가 전부 조용할 수 있다 — 실측:
    # 러셀2000이 자기 이력 상위 2%로 움직인 26일 중 **5일(19%)**이 그랬다.
    #
    # 관측 20곳이 전부 대형주라 여기 붙일 기업이 **하나도 없다.** 그래서 당일
    # 신고가 아니라 상시 사실로만 나간다 — 그리고 그것이 이 시스템의 가장 큰
    # 구조적 사각지대다. 지금까지는 그 사실조차 말하지 않았다.
    "^RUT": [],                                # 러셀2000(소형주) — 관측 0곳
    # --- 테마 축 (2026-08-20) ---
    #
    # 업종 지수가 **아니다**(`universe.py`의 테마 축 주석 참조) — 서로 겹치므로
    # 업종 표에는 안 싣는다. 다만 사각지대 판정에는 참여시킨다: 겹치더라도
    # "기존 축이 전부 조용한 날"을 실측으로 33~42% 만들어 내기 때문이다.
    "^SOX": ["NVDA", "TSM", "MU", "SNDK"],     # 반도체 — 신규 26%
    "IGV":  ["MSFT"],                          # 소프트웨어·플랫폼 — 신규 42%
    "ITA":  [],                                # 항공우주·방산 — 관측 0곳, 신규 38%
    "BOTZ": ["NVDA"],                          # 로보틱스·AI — 신규 16%
}
...
# 업종 지수가 아닌데 여기서 함께 보는 축. **명시로 적는다** — 시험이 이 집합과
# 정확히 대조하므로, 누가 조용히 더하거나 빼면 빨간불이 난다.
NON_SECTOR_AXES = {"^RUT", "^SOX", "IGV", "ITA", "BOTZ"}
