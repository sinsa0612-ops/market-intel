...
SERIES = [
    ...
    "DTWEXBGS",       # 광의 달러지수
    # 2026-08-14 추가. CEO가 "PPI가 예상을 하회했다"는 외부 서사를 가져왔는데
    # **PPI를 수집하지 않아 검증 자체가 불가능했다.** 소비자물가만 보고 생산자물가를
    # 안 보면 물가 압력이 소비자에게 오기 **전** 단계를 통째로 못 읽는다.
    # 뉴스가 "PPI"라 부르는 것은 최종수요 기준이므로 그것을 쓴다(전품목 `PPIACO`는
    # 1913년까지 있으나 헤드라인과 다른 숫자다 — 둘을 섞으면 화면이 거짓말한다).
    "PPIFIS",         # 생산자물가(최종수요)
]
